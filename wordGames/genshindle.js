...
// SFX toggle
let sfxEnabled = localStorage.getItem('genshindle_sfx_enabled') !== 'false';
...
const CHARACTERS_URL = '../data/genshin_characters_final.json';
...
// --- Stats System ---
const STATS_KEY = 'genshindle_stats';
...
function buildShareText(won) {
    const lines = [];
    lines.push(`Genshindle ${TODAY_DATE_STR} ${won ? guessesHistory.length : 'X'}/${MAX_GUESSES}`);
    lines.push('');

    const attrKeys = ['element', 'weapon', 'rarity', 'releaseVersion', 'region', 'gender'];
    guessesHistory.forEach(characterName => {
        const character = CHARACTERS.find(r => r.name === characterName);
        if (!character) return;
...
function runConfetti() {
    ...
    const pieces = Array.from({ length: 140 }, () => ({
        ...
        color: ['#d4af37', '#8b5cf6', '#22c55e', '#eab308', '#ef4444', '#00d4ff'][Math.floor(Math.random() * 6)],
...
btnSfx.addEventListener('click', () => {
    ...
    localStorage.setItem('genshindle_sfx_enabled', sfxEnabled);
    updateSfxButton();
...
function submitGuess() {
    ...
    let guessedCharacter = CHARACTERS.find(r => normalizeName(r.name) === normalizeName(val));

    if (!guessedCharacter) guessedCharacter = CHARACTERS.find(r => r.name.toLowerCase() === val.toLowerCase());

    if (!guessedCharacter) guessedCharacter = CHARACTERS.find(r => r.name.toLowerCase().startsWith(val.toLowerCase()) || r.name.toLowerCase().includes(val.toLowerCase()));
    ...
    if (isCorrect) {
        ...
        lastGameWon = true;
        Sound.win();
        toastEl.textContent = `Splendid! Genshindle Solved!`;
        inputEl.disabled = true;
        btnGuess.disabled = true;
...
function checkHintState() {
    if (guessesHistory.length >= 5 && !gameOver) {
        ...
        hintBox.innerHTML = `<i class="fa-solid fa-lightbulb"></i> <strong>Hint Unlocked:</strong> Role: <strong>${TARGET_CHARACTER.role}</strong>, starts with '<strong>${firstLetter}</strong>'!`;
    }
}
...
function renderRowUI(character, shouldAnimate = false) {
    ...
    const fields = [
        ...
        { key: 'weapon', val: character.weapon, label: 'Weapon' },
        { key: 'rarity', val: character.rarity, label: 'Rarity' },
        { key: 'releaseVersion', val: character.releaseVersion, label: 'Version' },
        { key: 'region', val: character.region, label: 'Region' }
    ];
...
function triggerVictoryModal(character) {
    ...
    winWeapon.textContent = character.weapon || 'Unknown';
    winRarity.textContent = character.rarity || 'Unknown';
    winRole.textContent = character.releaseVersion || 'Unknown';
    winFaction.textContent = character.region || 'Unknown';
...
function triggerGameOverModal(character) {
    ...
    loseWeapon.textContent = character.weapon || 'Unknown';
    loseRarity.textContent = character.rarity || 'Unknown';
    loseRole.textContent = character.releaseVersion || 'Unknown';
    loseFaction.textContent = character.region || 'Unknown';
...
function restoreProgress() {
    const saved = JSON.parse(localStorage.getItem(`genshindle_save_${TODAY_DATE_STR}`));
    if (!saved) return;
    ...
    if (gameOver) {
        ...
        btnGuess.disabled = true;
        if (saved.passed) {
            toastEl.textContent = `Daily Genshindle Solved!`;
        } else {
...
function saveProgress(passed = false) {
    localStorage.setItem(`genshindle_save_${TODAY_DATE_STR}`, JSON.stringify({
        date: TODAY_DATE_STR,
